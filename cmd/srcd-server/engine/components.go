...
func (s *Server) StartComponentWithLogs(
	r *api.StartComponentRequest,
	stream api.Engine_StartComponentWithLogsServer,
) error {
	log := func(level logrus.Level, msg string) {
		err := stream.Send(&api.StartComponentResponse{
			Kind: api.StartComponentResponse_LOG,
			Log:  api.NewLogEntry(level, msg),
		})
		if err != nil {
			logrus.Errorf("could not stream log: %v", err)
		}
	}
	ctx := setLogf(context.Background(), log)

	if err := s.startComponentAtPort(ctx, r.Name, int(r.Port)); err != nil {
		return err
	}

	return stream.Send(&api.StartComponentResponse{
		Kind: api.StartComponentResponse_FINAL,
	})
}
